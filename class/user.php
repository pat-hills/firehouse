...
class user {
   ...
   $pdo = $this->pdo;
   $stmt = $pdo->prepare('SELECT id,room_no,inter_com_no FROM tbl_rooms WHERE u_id = :u_id AND deleted = :N_O_O');
   $stmt ->bindParam(':u_id', $user_id);
...
public function get_user_firstName_by_userId(){
    ...
    try {
     ...
     if(is_null($this->pdo)){
         ...
         $stirngdel = "NO";
         $pdo = $this->pdo;
         $stmt = $pdo->prepare('SELECT firstName FROM tbl_staff WHERE staff_id = :u_id AND membership_id = :membership_id');
         $stmt ->bindParam(':u_id', $user_id);
