...
//session_start();
  ...
  function con_con (){
    ...
    // $passdb = "Systems.01.01";   
    // $namedb = "taugscsf_guesthouse";  
...
function member_details($member_id){
  ...
  $uuid =  $_SESSION['userid'];
 // $grp = $_SESSION['grp'];
  $query = "SELECT * FROM customers WHERE id = '$member_id' AND deleted = 'NO'";
...
function user_details($member_id){
             
  $db = con_con();
  $uuid =  $_SESSION['userid'];
 // $grp = $_SESSION['grp'];
  $query = "SELECT * FROM tbl_sub_user WHERE sub_id = '$member_id' AND deleted = 'NO'";
...
function get_payment_details_by_id($member_id){
  ...
  $uuid =  $_SESSION['userid'];
  //$grp = $_SESSION['grp'];
  $query = "SELECT * FROM tbl_bookings WHERE transaction_code = '{$member_id}' AND  u_id = '{$uuid}' AND deleted = 'NO'";
...
function sum_guest_receipt($id){
 // $initial_balance = 0.00;         
  ...
  $query = "SELECT SUM(amount_charged) FROM tbl_bookings WHERE transaction_code = '{$id}' AND deleted = 'NO'  ";
   $result= $db->query($query) or die("Error");
if($result){
if(mysqli_num_rows($result)>0){
...
function searchpayements(){
  ...
  $query = "SELECT * FROM tbl_bookings WHERE deleted = 'NO' AND DATE(book_date) >= '{$datefrom}' AND DATE(book_date) <= '{$dateto}' ";
   $result= $db->query($query) or die("Error");
if($result){
if(mysqli_num_rows($result)>0){
...
function searchpayementssum(){
  ...
  $query = "SELECT SUM(amount_charged) FROM tbl_bookings WHERE deleted = 'NO'
...
if(mysqli_num_rows($result)>0){
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    return  $row['SUM(amount_charged)'] + $initial_balance;
     }
