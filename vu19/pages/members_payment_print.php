...
$pageTitle = "<strong>" . htmlentities( " " . "DEBTORS AND CREDITORS AS AT  " . strtoupper(date("F j, Y"))) . "</strong>";
...
<td style="text-align: left; width: 45%;"><strong>Amount ( GHS )</strong></td>
...
while ($row = mysqli_fetch_array($allg)) {
    ...
    $cur_led = all_ledger_of_member($member_id);
    $cur_pay = all_payments_of_member($member_id);
...
if($get_balance_paid){
    ...
    <tbody>
        <tr>
            ...
            <td style="width: 40%; text-align: left;">' . htmlentities($formatted) . '</td>
           
 
   <td style="width: 45%; text-align: left;"> ' . htmlentities($get_balance_paid) . '</td>
