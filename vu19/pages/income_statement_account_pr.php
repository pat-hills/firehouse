...
//        $this->Cell(0, 10, date("d/m/Y h:i:sa") . " " . 'Page ' . $this->getAliasNumPage() . '/' . $this->getAliasNbPages(), 0, false, 'R', 0, '', 0, false, 'T', 'M');
        ...
        $horizontalLine = "<hr />";
        $company = "<strong><em>Spencam Group</em></strong>";
...
$pageTitle = "<strong>" . htmlentities( " " . "Guest House Bookings " . strtoupper("")) . "</strong>";
$pdf->writeHTML($pageTitle, true, 0, true, 0, 'L');
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
         ...
         <td style="text-align: left; width: 25%;"><strong>Room No</strong></td>
         <td style="text-align: left; width: 45%;"><strong>Description</strong></td>
...
while ($row = mysqli_fetch_array($searchpayements)) {
             ...
             $label = "Booking At ".$row["book_date"];  
    ...
    <tbody>
        <tr>
            ...
            <td style="text-align: left; width: 25%;">' . $room . '</td>
            
           <td style="width: 45%; text-align: left;">' . htmlentities($label) . '</td>

           <td style="width: 25%;text-align: left;">' .htmlentities("".number_format( $row["amount_charged"], 2, ".", ",")). '</td>
